...
def load_dynamic_industry_weights(industry_weights_file: str = None):
    """
    ...
    Args:
        industry_weights_file: Optional specific file name (e.g., "industry_weights_hardware.json")
                             If None, searches for default "industry_weights.json"
    ...
    if industry_weights_file:
        candidates = [
            ROOT / 'artifacts' / 'weights' / industry_weights_file,
            Path.cwd() / industry_weights_file
        ]
    else:
        candidates = [
    ...
    for industry_weights_path in candidates:
        if industry_weights_path.exists():
            try:
                with open(industry_weights_path, 'r') as f:
                    data = json.load(f)
                weights = data.get('weights', PERFORMANCE_VERTICAL_WEIGHTS)
                print(f"[INFO] Loaded {len(weights)} dynamic industry weights from {industry_weights_path.name}")
                return weights
...
def calculate_scores(df, weights, size_config=None, division_config: DivisionConfig = None):
    """
    ...
    Args:
        ...
        division_config (DivisionConfig, optional): Division configuration. If provided,
                                                    generates division-prefixed columns.
    ...
    prefix = ""
    if division_config:
        prefix = f"{division_config.display_name}_"
        # Ensure size weight is zero
        weights["size"] = 0.0
        if division_config.weights.get("size", 0.0) != 0.0:
            print(f"[WARN] Size weight forced to 0.0 for division {division_config.name}")
    ...
    score_cols_to_drop = [
        f'{prefix}vertical_score', f'{prefix}size_score', f'{prefix}adoption_score', 
        f'{prefix}relationship_score', f'{prefix}relationship_feature', 
        f'{prefix}ICP_score_raw', f'{prefix}ICP_score'
    ]
    # Also drop unprefixed versions for backward compatibility during transition
    if prefix:
        score_cols_to_drop.extend([
    ...
    if division_config:
        industry_weights = load_dynamic_industry_weights(division_config.industry_weights_file)
    else:
        industry_weights = load_dynamic_industry_weights()
    v_lower = df_clean[COL_INDUSTRY].astype(str).str.lower().str.strip()
    df_clean[f"{prefix}vertical_score"] = v_lower.map(industry_weights).fillna(0.3)
    ...
    #    NOTE: Size score is calculated but always zero-weighted per requirements.
    ...
    df_clean[f"{prefix}size_score"] = 0.5  # Neutral default score
    ...
    for condition, score in zip(conditions, scores):
        ...
        df_clean.loc[mask, f"{prefix}size_score"] = score

    # 3. Adoption Score: Composite of asset/seat investment and adoption profit.
    #    Uses division-specific column names if division_config provided:
    #      - {prefix}adoption_assets (weighted seats/assets for division's adoption goals)
    #      - {prefix}adoption_profit (Profit since 2023 for division's adoption goals)
    ...
    # Determine adoption column names
    adoption_assets_col = f"{prefix}adoption_assets" if prefix else COL_ADOPTION_ASSETS
    adoption_profit_col = f"{prefix}adoption_profit" if prefix else COL_ADOPTION_PROFIT
    
    if adoption_assets_col in df_clean.columns and adoption_profit_col in df_clean.columns:
        assets_series = pd.to_numeric(df_clean[adoption_assets_col], errors='coerce').fillna(0)
        profit_series = pd.to_numeric(df_clean[adoption_profit_col], errors='coerce').fillna(0)
        ...
        df_clean[f'{prefix}adoption_score'] = adoption_scores
    else:
    ...
    # 4. Relationship Score: prefer profit from relationship goals if present.
    #    For divisions that reuse hardware adoption, look for hardware adoption features.
    relationship_profit_col = f"{prefix}relationship_profit" if prefix else COL_RELATIONSHIP_PROFIT
    
    # Check if this division reuses hardware adoption for relationship
    if division_config and division_config.relationship_uses_hardware_adoption:
        # Use hardware adoption features as relationship score
        hw_adoption_assets_col = "Hardware_adoption_assets" if prefix else COL_ADOPTION_ASSETS
        hw_adoption_profit_col = "Hardware_adoption_profit" if prefix else COL_ADOPTION_PROFIT
        
        # Try to find hardware adoption features
        if hw_adoption_assets_col in df_clean.columns and hw_adoption_profit_col in df_clean.columns:
            hw_assets = pd.to_numeric(df_clean[hw_adoption_assets_col], errors='coerce').fillna(0)
            hw_profit = pd.to_numeric(df_clean[hw_adoption_profit_col], errors='coerce').fillna(0)
            hw_total = hw_assets + hw_profit
            df_clean[f'{prefix}relationship_score'] = min_max_scale(np.log1p(np.maximum(hw_total, 0)))
        else:
            # Fallback: use zero if hardware adoption not available
            df_clean[f'{prefix}relationship_score'] = 0.0
    elif relationship_profit_col in df_clean.columns:
        rel_safe = np.maximum(pd.to_numeric(df_clean[relationship_profit_col], errors='coerce').fillna(0), 0)
        df_clean[f'{prefix}relationship_score'] = min_max_scale(np.log1p(rel_safe))
    else:
        relationship_cols = [COL_REL_LICENSE, COL_REL_SAAS, COL_REL_MAINT]
        for col in relationship_cols:
            if col not in df_clean.columns:
                df_clean[col] = 0
        df_clean[f'{prefix}relationship_feature'] = df_clean[relationship_cols].fillna(0).sum(axis=1)
        relationship_feature_safe = np.maximum(df_clean[f'{prefix}relationship_feature'], 0)
        df_clean[f'{prefix}relationship_score'] = min_max_scale(np.log1p(relationship_feature_safe))
    ...
    # Force size weight to 0.0 regardless of config
    weights["size"] = 0.0
    df_clean[f'{prefix}ICP_score_raw'] = (
        weights["vertical"] * df_clean[f'{prefix}vertical_score'] +
        weights["size"] * df_clean[f'{prefix}size_score'] +
        weights["adoption"] * df_clean[f'{prefix}adoption_score'] +
        weights["relationship"] * df_clean[f'{prefix}relationship_score']
    ) * 100
    ...
    ranks = df_clean[f'{prefix}ICP_score_raw'].rank(method='first')
    n = len(ranks)
    p = (ranks - 0.5) / n
    z = norm.ppf(p)

    # Scale the Z-scores to a 0-100 range with a mean of 50 and std dev of 15.
    df_clean[f'{prefix}ICP_score'] = (50 + 15 * z).clip(0, 100)
    
    # Assign letter grades based on the final normalized ICP score percentiles.
    df_clean[f'{prefix}ICP_grade'] = calculate_grades(df_clean[f'{prefix}ICP_score'])
