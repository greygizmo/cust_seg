"""
Core logic for generating playbooks and tags.
"""
...
from icp.schema import COL_CUSTOMER_ID, COL_COMPANY_NAME
from icp.playbooks.definitions import PlaybookDefinition
...
def flag_heroes(scored: pd.DataFrame) -> pd.Series:
...
def compute_neighbor_flags(scored: pd.DataFrame, neighbors: pd.DataFrame) -> pd.DataFrame:
    """Add simple neighbor-based features: hero look-alike and orphan look-alike flags."""

    scored = scored.copy()
    scored["is_hero"] = flag_heroes(scored)
...
def build_playbook_masks(scored: pd.DataFrame) -> Tuple[Dict[str, pd.Series], Dict[str, PlaybookDefinition]]:
...
def derive_playbooks(scored_with_flags: pd.DataFrame) -> pd.DataFrame:
    masks, defs = build_playbook_masks(scored_with_flags)
